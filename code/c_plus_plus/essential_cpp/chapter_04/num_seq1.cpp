...
const int NumSeqVer1::max_seq = 64;
...
{
    if (pos <= 0 || pos + beg_pos_ - 1 > max_seq)
    {
...
{
    if (pos <= 0 || pos > max_seq || pos >= beg_pos_ + length_)
    {
...
ostream &NumSeqVer1::Print(ostream &os) const
{
...
bool NumSeqVer1::End(NumSeqVer1::iterator &iter)
{
    if (!CheckIntegrity(length_ + beg_pos_ -1))
        return false;
...
ostream &operator<<(ostream &os, const NumSeqVer1 &ns)
{
    ns.Print(os);
    return os;
}
