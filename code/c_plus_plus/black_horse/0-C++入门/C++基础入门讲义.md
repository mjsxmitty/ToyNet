...
**作用：**用于表示一些**不能显示出来的ASCII字符**，现阶段我们常用的转义字符有：` \n  \\  \t`
...
```
> 总结：只有整型变量可以进行取模运算，在除法运算中，除数不能为0
...
**案例描述：**水仙花数是指一个 3 位数，它的每个位上的数字的3次幂之和等于它本身，例如：1^3 + 5^3+ 3^3 = 153。请利用do...while语句，求出所有3位数中的水仙花数
...
**作用：** 在循环体中再嵌套一层循环，解决一些实际问题。例如我们想在屏幕中打印如下图片，就需要利用嵌套循环
